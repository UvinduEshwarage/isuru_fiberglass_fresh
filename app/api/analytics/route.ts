...
import { invoiceCollection, ensureInvoiceIndexes } from "../../../lib/invoiceModel";
...
export async function GET(request: NextRequest) {
  ...
  const records: MlTrendRecord[] = Object.entries(monthlyRevenue).map(([month, revenue]) => ({
    date: `${month}-01`,
    revenue,
  }));

  const trendAnalysis = await fetchTrendAnalysis(records);
